...
import database "github.com/thanhchungbtc/mywallet/server/app/database"
...
func (_m *Auth) ParseToken(tokenStr string) (*database.Claims, error) {
	ret := _m.Called(tokenStr)

	var r0 *database.Claims
	if rf, ok := ret.Get(0).(func(string) *database.Claims); ok {
		r0 = rf(tokenStr)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*database.Claims)
		}
	}
